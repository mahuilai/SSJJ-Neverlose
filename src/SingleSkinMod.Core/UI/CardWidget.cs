...
namespace SingleSkinMod.UI
{
    ...
    public sealed class CardScrollBar
    {
        ...
        public float Draw(Rect viewport, float current, float target, float contentHeight)
        {
            ...
            Rect track = new Rect(viewport.xMax - 9f, viewport.y + 5f, 6f, viewport.height - 10f);
            float thumbHeight = Mathf.Clamp(track.height * (viewport.height / contentHeight), 38f, track.height);
            float travel = Mathf.Max(0f, track.height - thumbHeight);
            float thumbY = track.y + travel * Mathf.Clamp01(current / maxScroll);
            Rect thumb = new Rect(track.x - 1f, thumbY, 8f, thumbHeight);
            Rect trackHit = new Rect(track.x - 10f, track.y, 26f, track.height);
            Rect thumbHit = new Rect(thumb.x - 8f, thumb.y, thumb.width + 16f, thumb.height);
            ...
            if (hovered || _dragging)
                CardDraw.RoundedRect(new Rect(thumb.x - 3f, thumb.y, thumb.width + 6f, thumb.height), new Color(CardColors.Accent.r, CardColors.Accent.g, CardColors.Accent.b, 0.22f), 6f);
