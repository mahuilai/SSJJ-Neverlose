...
using SingleSkinMod.Entity;
using SingleSkinMod.UI;
using SingleSkinMod.Common;

namespace SingleSkinMod.Hud
{
    public sealed class CsgoHud : MonoBehaviour
    {
        ...
        {
            string[] candidates = {
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "SingleSkinMod", "CSGO_HUD"),
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "CSGO_HUD"),
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "BepInEx", "plugins", "SingleSkinMod", "CSGO_HUD"),
                @"D:\项目\单美化\assets\CSGO_HUD"
            };
        ...
        public static CsgoHud Instance { get; private set; }

        public void TriggerTestKill()
        {
            _localKillCombo = (_localKillCombo % 4) + 1;
            _lastLocalKillAt = Time.unscaledTime;
            _lastLocalKillVictim = "TestDummy_" + _localKillCombo;
            _killCard = new KillCardState
            {
                Victim = _lastLocalKillVictim,
                Headshot = true,
                Combo = _localKillCombo,
                CreatedAt = Time.unscaledTime
            };
            _notices.Insert(0, new KillNotice
            {
                Killer = "LocalPlayer",
                Victim = _lastLocalKillVictim,
                Weapon = "ak47_glory",
            ...
            TryPlayCustomKillSound(true, _localKillCombo, false);
        }
        ...
        private void Update()
        {
            if (!(SingleSkinMod.Plugin.ConfigInstance?.CsgoHudEnabled ?? true))
            {
                if (_wasEnabled)
                {
                    ClosePauseMenu();
        ...
        {
            if (!(SingleSkinMod.Plugin.ConfigInstance?.CsgoHudEnabled ?? true) || PlayerUpdate.LocalEntity == null)
                return;
        ...
        private void SetupKillAudio()
        {
            try
            {
                ...
                _killAudioObject = new GameObject("_killAudio");
        ...
        private static UnityEngine.Object LoadPcmWav(string path, Type clipType)
        {
            ...
            UnityEngine.Object clip = create.Invoke(null,
                new object[] { "_killGlass", frameCount, (int)channels, sampleRate, false }) as UnityEngine.Object;
