...
static bool g_csgoHudSuppress = false;
...
static float g_thirdPersonDistance = 220.0f;
static float g_thirdPersonFov = 90.0f;

// 3D 世界相机真实飘雪状态
static bool g_worldSnowEnabled = false;
static float g_worldSnowDensity = 200.0f;
static float g_worldSnowSpeed = 3.5f;
static float g_worldSnowSize = 0.15f;
...
static int g_subtabs[8] = { 0 };
static char g_weaponSearch[128] = "";
static char g_charSearch[128] = "";
static char g_accSearch[128] = "";
...
// Zero-hook input: We do NOT hook GWLP_WNDPROC to stay completely invisible
// to anti-cheat scanners (e.g. EyeRecall.dll CheckEnumWindowPro).
// Input is driven cleanly via GetCursorPos and GetAsyncKeyState in HookPresent.

// ===== Neverlose Fonts & Styling =====
static void SetupNeverloseStyle(float scale = 2.0f) {
    ImGuiStyle& s = ImGui::GetStyle();
    s.WindowRounding = 6.0f * scale;
    s.ChildRounding = 4.0f * scale;
    s.FrameRounding = 4.0f * scale;
    s.PopupRounding = 4.0f * scale;
    s.GrabRounding = 4.0f * scale;
    s.ScrollbarRounding = 4.0f * scale;
    s.WindowBorderSize = 1.0f * scale;
    s.FrameBorderSize = 0.0f;
    s.WindowPadding = ImVec2(0.0f, 0.0f);
    s.FramePadding = ImVec2(8.0f * scale, 4.0f * scale);
    s.ItemSpacing = ImVec2(8.0f * scale, 8.0f * scale);
    s.ItemInnerSpacing = ImVec2(6.0f * scale, 4.0f * scale);
    s.ScrollbarSize = 6.0f * scale;
...
static void LoadNeverloseFonts(float scale = 2.0f) {
    ...
    for (const char* path : fonts) {
        if (GetFileAttributesA(path) != INVALID_FILE_ATTRIBUTES) {
            io.Fonts->AddFontFromFileTTF(path, 15.0f * scale, nullptr, io.Fonts->GetGlyphRangesChineseFull());
            baseLoaded = true;
            break;
        }
    }
    ...
    io.Fonts->AddFontFromMemoryTTF((void*)font_awesome_binary, sizeof(font_awesome_binary), 13.0f * scale, &icons_config, icon_ranges);

    // 3. Title font (Museo 900)
    io.Fonts->AddFontFromMemoryTTF((void*)museo900_binary, sizeof(museo900_binary), 22.0f * scale);
}

// ===== Draw Neverlose Menu =====
static void DrawNeverloseMenu() {
    float S = gui.m_scale;
    ImVec2 targetSize = ImVec2(780.0f * S, 530.0f * S);
    ImGui::SetNextWindowSize(targetSize, ImGuiCond_Always);

    ImVec2 disp = ImGui::GetIO().DisplaySize;
    if (disp.x > 0 && disp.y > 0) {
        float posX = (disp.x - targetSize.x) * 0.5f;
        float posY = (disp.y - targetSize.y) * 0.5f;
        if (posX < 10.0f) posX = 10.0f;
        if (posY < 10.0f) posY = 10.0f;
        ImGui::SetNextWindowPos(ImVec2(posX, posY), ImGuiCond_Once);
    } else {
        ImGui::SetNextWindowPos(ImVec2(60.0f, 40.0f), ImGuiCond_Once);
    }
    ...
    draw->AddRectFilled(pos, pos + size, ImColor(8, 14, 24, 250), 8.0f * S);
    draw->AddRect(pos, pos + size, gui.border.to_im_color(0.5f), 8.0f * S);

    // Sidebar vertical background & line
    draw->AddRectFilled(pos, pos + ImVec2(170.0f * S, size.y), ImColor(5, 9, 16, 255), 8.0f * S, ImDrawFlags_RoundCornersLeft);
    draw->AddLine(pos + ImVec2(170.0f * S, 0), pos + ImVec2(170.0f * S, size.y), gui.border.to_im_color());
    ...
    float titleSize = 22.0f * S;
    ImVec2 logoSize = titleFont->CalcTextSizeA(titleSize, FLT_MAX, 0.0f, logoText);
    draw->AddText(titleFont, titleSize, pos + ImVec2(85.0f * S - logoSize.x / 2.0f + 1, 16.0f * S), gui.accent_color.to_im_color(), logoText);
    draw->AddText(titleFont, titleSize, pos + ImVec2(85.0f * S - logoSize.x / 2.0f, 16.0f * S), gui.text.to_im_color(), logoText);

    // Sidebar navigation tabs
    ImGui::SetCursorPos(ImVec2(10.0f * S, 55.0f * S));
    ImGui::BeginChild("##SidebarTabs", ImVec2(150.0f * S, size.y - 120.0f * S), false, ImGuiWindowFlags_NoScrollbar);
    {
        gui.group_title("MODELS & SKINS");
        if (gui.tab(ICON_FA_CROSSHAIRS, "武器模型", gui.m_tab == 0) && gui.m_tab != 0) {
            gui.m_tab = 0; gui.m_anim = 0.0f;
        }
        if (gui.tab(ICON_FA_SHIELD_ALT, "人物角色", gui.m_tab == 1) && gui.m_tab != 1) {
            gui.m_tab = 1; gui.m_anim = 0.0f;
        }
        if (gui.tab(ICON_FA_MAGIC, "背饰挂件", gui.m_tab == 2) && gui.m_tab != 2) {
            gui.m_tab = 2; gui.m_anim = 0.0f;
        }
        if (gui.tab(ICON_FA_PAINT_BRUSH, "自定义材质", gui.m_tab == 3) && gui.m_tab != 3) {
            gui.m_tab = 3; gui.m_anim = 0.0f;
        }

        ImGui::Spacing(); ImGui::Spacing();
        gui.group_title("VISUAL & COMBAT");
        if (gui.tab(ICON_FA_PALETTE, "雨爱光影", gui.m_tab == 4) && gui.m_tab != 4) {
            gui.m_tab = 4; gui.m_anim = 0.0f;
        }
        if (gui.tab(ICON_FA_DESKTOP, "战斗 HUD", gui.m_tab == 5) && gui.m_tab != 5) {
            gui.m_tab = 5; gui.m_anim = 0.0f;
        }

        ImGui::Spacing(); ImGui::Spacing();
        gui.group_title("CONFIGURATION");
        if (gui.tab(ICON_FA_SLIDERS_H, "玩家微调", gui.m_tab == 6) && gui.m_tab != 6) {
            gui.m_tab = 6; gui.m_anim = 0.0f;
        }
    }
    ImGui::EndChild();

    // Sidebar user/status footer
    draw->AddLine(pos + ImVec2(0, size.y - 52.0f * S), pos + ImVec2(170.0f * S, size.y - 52.0f * S), gui.border.to_im_color());
    draw->AddCircleFilled(pos + ImVec2(24.0f * S, size.y - 26.0f * S), 10.0f * S, gui.accent_color.to_im_color(0.3f));
    draw->AddCircleFilled(pos + ImVec2(24.0f * S, size.y - 26.0f * S), 5.0f * S, gui.accent_color.to_im_color());
    draw->AddText(pos + ImVec2(42.0f * S, size.y - 36.0f * S), gui.text.to_im_color(), "SSJJ SingleSkin");
    {
        std::lock_guard<std::mutex> lock(g_stateMutex);
        std::string st = WstringToUtf8(g_status);
        if (st.length() > 14) st = st.substr(0, 14) + "..";
        draw->AddText(pos + ImVec2(42.0f * S, size.y - 20.0f * S), gui.text_disabled.to_im_color(), st.c_str());
    }

    // Top action bar (Quick Apply F5 / Restore F6 & Subtabs)
    ImGui::SetCursorPos(ImVec2(185.0f * S, 14.0f * S));
    if (ImGui::Button(ICON_FA_SAVE " 一键应用 (F5)", ImVec2(140.0f * S, 28.0f * S))) {
        QueueCommand(CmdApplyAll);
    }
    ImGui::SameLine();
    if (ImGui::Button(ICON_FA_UNDO " 还原 (F6)", ImVec2(110.0f * S, 28.0f * S))) {
    ...
    if (gui.m_tab == 4) {
        ...
        subtabLabels = { "玩家参数微调", "第三人称视角", "系统与快捷键" };
    }

    int activeSub = g_subtabs[gui.m_tab];
    if (!subtabLabels.empty()) {
        if (activeSub >= (int)subtabLabels.size()) activeSub = 0;

        ImGui::SameLine(ImGui::GetWindowWidth() - 320.0f * S);
        ImGui::BeginChild("##SubtabBar", ImVec2(305.0f * S, 28.0f * S), false, ImGuiWindowFlags_NoScrollbar);
        {
            draw->AddRectFilled(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImGui::GetWindowSize(), gui.button.to_im_color(), 4.0f * S);
            draw->AddRect(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImGui::GetWindowSize(), gui.border.to_im_color(), 4.0f * S);
    ...
    // Main content area
    float contentW = size.x - 195.0f * S;
    float contentH = size.y - 62.0f * S;
    float colW = (contentW - 10.0f * S) / 2.0f;

    ImGui::SetCursorPos(ImVec2(185.0f * S, 50.0f * S));
    ImGui::BeginChild("##MainContent", ImVec2(contentW, contentH), false, ImGuiWindowFlags_NoScrollbar);
    {
        ...
        if (gui.m_tab == 0) {
            gui.group_box(ICON_FA_CROSSHAIRS " 官方武器模型库 (ECS 热切换)", ImVec2(colW, contentH));
            {
                ...
                ImGui::PushStyleVar(ImGuiStyleVar_ScrollbarRounding, 6.0f * S);
                ImGui::BeginChild("##WeaponCatalogList", ImVec2(0, contentH - 85.0f * S), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);
                std::lock_guard<std::mutex> lock(g_stateMutex);
            ...
            gui.group_box(ICON_FA_CHECK " 当前选定武器与控制", ImVec2(colW, contentH));
            {
                ...
                if (ImGui::Button(ICON_FA_SAVE " 立即应用换模 (F5)", ImVec2(colW - 20.0f * S, 36.0f * S))) {
                    QueueCommand(CmdApplyAll);
                }
                ImGui::Spacing();
                if (ImGui::Button(ICON_FA_UNDO " 还原官方原厂武器 (F6)", ImVec2(colW - 20.0f * S, 36.0f * S))) {
                    QueueCommand(CmdRestore);
                }
            }
            gui.end_group_box();
        ...
        // ===== TAB 1: CHARACTER SWAP =====
            ...
            gui.group_box(ICON_FA_SHIELD_ALT " 官方角色模型库 (ECS 热替换)", ImVec2(colW, contentH));
            {
                ImGui::InputTextWithHint("##CharSearch", "输入搜索角色名称...", g_charSearch, sizeof(g_charSearch));
                ImGui::Spacing();

                std::string filter = g_charSearch;
                std::transform(filter.begin(), filter.end(), filter.begin(), ::tolower);

                ImGui::PushStyleVar(ImGuiStyleVar_ScrollbarSize, 14.0f * S);
                ImGui::PushStyleVar(ImGuiStyleVar_ScrollbarRounding, 6.0f * S);
                ImGui::BeginChild("##CharCatalogList", ImVec2(0, contentH - 85.0f * S), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);
                std::lock_guard<std::mutex> lock(g_stateMutex);
                const auto& list = g_catalogs[1];
                std::string currentSel = WstringToUtf8(g_selections[1]);

                if (list.empty()) {
                    ImGui::TextDisabled("角色目录扫描中，局内自动就绪...");
                } else {
                    for (const auto& witem : list) {
                        std::string item = WstringToUtf8(witem);
                        std::string itemLower = item;
            ...
            gui.group_box(ICON_FA_CHECK " 当前选定角色与控制", ImVec2(colW, contentH));
            {
                std::string currentSel;
                {
                    std::lock_guard<std::mutex> lock(g_stateMutex);
                    currentSel = WstringToUtf8(g_selections[1]);
                }
                ImGui::TextColored(gui.accent_color.to_im_color(), "当前选定角色:");
                ImGui::TextWrapped("%s", currentSel.empty() ? "(未选定)" : currentSel.c_str());
                ImGui::Spacing(); ImGui::Separator(); ImGui::Spacing();

                ImGui::TextDisabled("说明: 在左侧列表点击选定心仪人物（如各类女角色/幻影角色），点击下方 [立即应用角色换模] 或在局内按 [F5] 即可瞬间完成换模。");
                ImGui::Spacing();

                if (ImGui::Button(ICON_FA_SAVE " 立即应用角色换模 (F5)", ImVec2(colW - 20.0f * S, 36.0f * S))) {
                    QueueCommand(CmdApplyAll);
                }
                ImGui::Spacing();
                if (ImGui::Button(ICON_FA_UNDO " 还原官方默认角色 (F6)", ImVec2(colW - 20.0f * S, 36.0f * S))) {
                    QueueCommand(CmdRestore);
                }
            }
            gui.end_group_box();
        }
        // ===== TAB 2: ACCESSORY SWAP =====
        else if (gui.m_tab == 2) {
            gui.group_box(ICON_FA_MAGIC " 翅膀 / 背部挂件库 (热挂载)", ImVec2(colW, contentH));
            {
                ImGui::InputTextWithHint("##AccSearch", "输入搜索挂件名称...", g_accSearch, sizeof(g_accSearch));
                ImGui::Spacing();

                std::string filter = g_accSearch;
                std::transform(filter.begin(), filter.end(), filter.begin(), ::tolower);

                ImGui::PushStyleVar(ImGuiStyleVar_ScrollbarSize, 14.0f * S);
                ImGui::PushStyleVar(ImGuiStyleVar_ScrollbarRounding, 6.0f * S);
                ImGui::BeginChild("##AccCatalogList", ImVec2(0, contentH - 85.0f * S), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);
                std::lock_guard<std::mutex> lock(g_stateMutex);
                const auto& list = g_catalogs[2];
                std::string currentSel = WstringToUtf8(g_selections[2]);

                if (list.empty()) {
                    ImGui::TextDisabled("挂件目录扫描中，局内自动就绪...");
                } else {
                    for (const auto& witem : list) {
                        std::string item = WstringToUtf8(witem);
                        std::string itemLower = item;
            ...
            gui.group_box(ICON_FA_CHECK " 当前选定挂件与控制", ImVec2(colW, contentH));
            {
                std::string currentSel;
                {
                    std::lock_guard<std::mutex> lock(g_stateMutex);
                    currentSel = WstringToUtf8(g_selections[2]);
                }
                ImGui::TextColored(gui.accent_color.to_im_color(), "当前选定背饰/翅膀:");
                ImGui::TextWrapped("%s", currentSel.empty() ? "(未装备 / 官方默认)" : currentSel.c_str());
                ImGui::Spacing(); ImGui::Separator(); ImGui::Spacing();

                ImGui::TextDisabled("说明: 选择目标背饰或战斗机翼/喷气背包，点击下方 [立即装备背饰换模] 或按 [F5] 即可瞬间挂载。若不需要背饰可点击 [卸下背部挂件]。");
                ImGui::Spacing();

                if (ImGui::Button(ICON_FA_SAVE " 立即装备背饰换模 (F5)", ImVec2(colW - 20.0f * S, 36.0f * S))) {
                    QueueCommand(CmdApplyAll);
                }
                ImGui::Spacing();
                if (ImGui::Button(ICON_FA_UNDO " 卸下背部挂件 / 清空", ImVec2(colW - 20.0f * S, 36.0f * S))) {
                    {
                        std::lock_guard<std::mutex> lock(g_stateMutex);
                        g_selections[2] = L"";
                    }
                    QueueCommand(CmdAccessory, 0.0f, L"");
                    QueueCommand(CmdApplyAll);
                }
            }
            gui.end_group_box();
        }
        // ===== TAB 3: CUSTOM PNG & MAP =====
        else if (gui.m_tab == 3) {
            gui.group_box(ICON_FA_PAINT_BRUSH " 自定义 PNG 材质热覆盖", ImVec2(colW, contentH));
            {
                ...
                if (ImGui::Button("执行材质替换 (DoSwap)", ImVec2(colW - 20.0f * S, 36.0f * S))) {
            ...
            gui.group_box(ICON_FA_PALETTE " 地图高清纹理全套批量注入", ImVec2(colW, contentH));
            {
                ...
                if (ImGui::Button("一键注入整套地图贴图", ImVec2(colW - 20.0f * S, 36.0f * S))) {
        ...
        // ===== TAB 4: VISUAL & WEATHER (雨爱光影) =====
            ...
            if (activeSub == 0) {
                ...
                gui.group_box(ICON_FA_MAGIC " 雨爱调色大师预设", ImVec2(colW, contentH));
                {
                    ...
                    if (ImGui::Button("1. 赛博紫夜 (Cyber Purple)", ImVec2(colW - 20.0f * S, 30.0f * S))) {
                    ...
                    ImGui::Spacing();
                    if (ImGui::Button("2. 暮光暖阳 (Sunset Glow)", ImVec2(colW - 20.0f * S, 30.0f * S))) {
                    ...
                    ImGui::Spacing();
                    if (ImGui::Button("3. 冰晶极地 (Arctic Ice)", ImVec2(colW - 20.0f * S, 30.0f * S))) {
                    ...
                    ImGui::Spacing();
                    if (ImGui::Button("4. 幽荧暗绿 (Ghostly Green)", ImVec2(colW - 20.0f * S, 30.0f * S))) {
                    ...
                    ImGui::Spacing();
                    if (ImGui::Button("5. 重置默认原版色彩 (Reset)", ImVec2(colW - 20.0f * S, 30.0f * S))) {
                ...
                // 浪漫飘雪粒子 (2D 屏幕粒子与 3D 世界相机粒子)
                gui.group_box(ICON_FA_SNOWFLAKE " 雨爱 2D 屏幕落雪系统 (ScreenSnow)", ImVec2(colW, contentH));
                {
                    if (ImGui::Checkbox("启用 2D 屏幕落雪", &g_snowEnabled)) {
                        QueueCommand(CmdSnowCount, g_snowEnabled ? g_snowCount : 0.0f);
                    }
                    ImGui::Spacing();

                    if (ImGui::SliderFloat("2D 雪花粒子密度 (Count)", &g_snowCount, 10.0f, 150.0f, "%.0f")) {
                        if (g_snowEnabled) QueueCommand(CmdSnowCount, g_snowCount);
                    }
                    if (ImGui::SliderFloat("2D 飘落速度 (Speed)", &g_snowSpeed, 10.0f, 120.0f, "%.1f")) {
                        QueueCommand(CmdSnowSpeed, g_snowSpeed);
                    }
                    if (ImGui::SliderFloat("2D 雪花平均尺寸 (Size)", &g_snowSize, 2.0f, 12.0f, "%.1f")) {
                        QueueCommand(CmdSnowSize, g_snowSize);
                    }

                    ImGui::Spacing(); ImGui::Separator(); ImGui::Spacing();
                    ImGui::TextDisabled("2D 屏幕半透明羽化算法与漂移物理模拟，直接附着于游戏视窗表面。");
                }
                gui.end_group_box();

                ImGui::SameLine();

                gui.group_box(ICON_FA_MAGIC " 3D 世界镜头空间降雪 (WorldSnow)", ImVec2(colW, contentH));
                {
                    if (ImGui::Checkbox("启用 3D 镜头真实空间降雪", &g_worldSnowEnabled)) {
                        QueueCommand(CmdWorldSnowToggle, g_worldSnowEnabled ? 1.0f : 0.0f);
                    }
                    ImGui::Spacing();

                    if (ImGui::SliderFloat("3D 粒子发射密度 (Density)", &g_worldSnowDensity, 20.0f, 600.0f, "%.0f")) {
                        QueueCommand(CmdWorldSnowDensity, g_worldSnowDensity);
                    }
                    if (ImGui::SliderFloat("3D 下落飘逸速度 (Speed)", &g_worldSnowSpeed, 0.5f, 15.0f, "%.1f")) {
                        QueueCommand(CmdWorldSnowSpeed, g_worldSnowSpeed);
                    }
                    if (ImGui::SliderFloat("3D 雪花颗粒尺寸 (Size)", &g_worldSnowSize, 0.05f, 0.60f, "%.2f")) {
                        QueueCommand(CmdWorldSnowSize, g_worldSnowSize);
                    }

                    ImGui::Spacing(); ImGui::Separator(); ImGui::Spacing();
                    ImGui::TextColored(gui.accent_color.to_im_color(), "3D 世界空间特性:");
                    ImGui::BulletText("附着于 Camera.main 空间粒子系统");
                    ImGui::BulletText("随视点移动具备真实立体视差与微风扰动");
                    ImGui::BulletText("无论远景还是近战，享受唯美沉浸感");
                }
                gui.end_group_box();
            }
            else {
                ...
                gui.group_box(ICON_FA_COG " 后处理说明", ImVec2(colW, contentH));
                {
                    ...
                    ImGui::Spacing();
                    if (ImGui::Button("重置后处理参数", ImVec2(colW - 20.0f * S, 32.0f * S))) {
        ...
        // ===== TAB 5: COMBAT HUD =====
            ...
            if (activeSub == 0) {
                ...
                gui.group_box(ICON_FA_DESKTOP " 界面特性与测试", ImVec2(colW, contentH));
                {
                    ...
                    if (ImGui::Button(ICON_FA_CROSSHAIRS " 模拟测试击杀播报 (Test Kill)", ImVec2(colW - 20.0f * S, 32.0f * S))) {
                        QueueCommand(CmdCsgoHudTestKill);
                    }
                    ImGui::Spacing();
                    ImGui::TextDisabled("点击立即触发一次测试击杀播报与玻璃击碎音效。");
                }
                gui.end_group_box();
            }
            else {
        ...
        // ===== TAB 6: SETTINGS =====
            ...
            if (activeSub == 0) {
                ...
                gui.group_box(ICON_FA_SHIELD_ALT " 属性应用说明", ImVec2(colW, contentH));
                {
                    ...
                    ImGui::Spacing();
                    if (ImGui::Button("复位默认参数", ImVec2(colW - 20.0f * S, 32.0f * S))) {
                        std::lock_guard<std::mutex> lock(g_stateMutex);
                ...
                gui.group_box(ICON_FA_CAMERA " 第三人称原生视角系统 (TPS Camera)", ImVec2(colW, contentH));
                {
                    if (ImGui::Checkbox("启用第三人称模式 (可按 [F3] 瞬切)", &g_thirdPersonEnabled)) {
                        QueueCommand(CmdThirdPerson, g_thirdPersonEnabled ? 1.0f : 0.0f);
                    }
                    ImGui::Spacing();

                    if (ImGui::SliderFloat("摄像机后拉距离 (Distance)", &g_thirdPersonDistance, 100.0f, 400.0f, "%.0f")) {
                        QueueCommand(CmdThirdPersonDistance, g_thirdPersonDistance);
                    }
                    if (ImGui::SliderFloat("镜头广角视野 (FOV)", &g_thirdPersonFov, 60.0f, 120.0f, "%.0f")) {
                        QueueCommand(CmdThirdPersonFov, g_thirdPersonFov);
                    }

                    ImGui::Spacing(); ImGui::Separator(); ImGui::Spacing();
                    if (ImGui::Button("重置标准距离与视野 (220 / 90)", ImVec2(colW - 20.0f * S, 32.0f * S))) {
                        g_thirdPersonDistance = 220.0f;
                        g_thirdPersonFov = 90.0f;
                        QueueCommand(CmdThirdPersonDistance, 220.0f);
                        QueueCommand(CmdThirdPersonFov, 90.0f);
                    }
                    ImGui::Spacing();
                    ImGui::TextDisabled("任何时刻均可随时按快捷键 [F3] 瞬切视角。");
                }
                gui.end_group_box();

                ImGui::SameLine();

                gui.group_box(ICON_FA_SHIELD_ALT " 原生 TPS 架构与防遮挡优势", ImVec2(colW, contentH));
                {
                    ImGui::BulletText("挂钩官方底层 TpsCameraLogic 引擎");
                    ImGui::BulletText("完美支持换模角色全身、背饰与武器");
                    ImGui::BulletText("原生墙面自动前推防穿模防遮挡");
                    ImGui::BulletText("已深度打补丁屏蔽反作弊上报与闪退");
                    ImGui::Spacing(); ImGui::Separator(); ImGui::Spacing();
                    ImGui::TextColored(gui.accent_color.to_im_color(), "操作提示:");
                    ImGui::TextWrapped("进入房间或正在战斗时按 F3，即可立刻由 FPS 第一人称变身为第三人称全身视角。");
                }
                gui.end_group_box();
            }
            else {
                gui.group_box(ICON_FA_DESKTOP " 系统连接与状态", ImVec2(colW, contentH));
                {
                    ...
                    ImGui::BulletText("[F3] : 瞬切第三人称 / 第一人称视角");
...
static HRESULT __stdcall HookPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags) {
    if (!g_rendererReady && pSwapChain) {
        ...
        if (SUCCEEDED(pSwapChain->GetDevice(IID_PPV_ARGS(&g_d3d11Device)))) {
            ...
            SetupNeverloseStyle(gui.m_scale);
            LoadNeverloseFonts(gui.m_scale);

            ImGui_ImplWin32_Init(g_window);
            ImGui_ImplDX11_Init(g_d3d11Device, g_d3d11Context);

            g_originalWndProc = (WNDPROC)GetWindowLongPtrW(g_window, GWLP_WNDPROC);
            g_rendererReady = true;
        }
    }

    if (g_rendererReady && !g_shuttingDown) {
        // Hotkey polling via GetAsyncKeyState (stealth, zero hook)
        static bool s_homeKeyPrev = false;
        bool homeDown = (GetAsyncKeyState(VK_HOME) & 0x8000) != 0 || (GetAsyncKeyState(VK_F12) & 0x8000) != 0;
        if (homeDown && !s_homeKeyPrev) {
            g_menuVisible = !g_menuVisible;
        }
        s_homeKeyPrev = homeDown;

        static bool s_f3Prev = false;
        ...
        if (g_menuVisible) {
            // Draw mouse cursor via ImGui so user sees it even if Unity locks OS cursor
            io.MouseDrawCursor = true;

            // 1. Mouse Position via GetCursorPos & ScreenToClient
            POINT cursorPos;
            if (GetCursorPos(&cursorPos) && ScreenToClient(g_window, &cursorPos)) {
            ...
            // 3. Modifiers & Special Keys
            bool shift = (GetAsyncKeyState(VK_SHIFT) & 0x8000) != 0;
            io.AddKeyEvent(ImGuiKey_LeftShift, shift);
            io.AddKeyEvent(ImGuiKey_LeftCtrl, (GetAsyncKeyState(VK_CONTROL) & 0x8000) != 0);
            io.AddKeyEvent(ImGuiKey_LeftAlt, (GetAsyncKeyState(VK_MENU) & 0x8000) != 0);
            io.AddKeyEvent(ImGuiKey_Backspace, (GetAsyncKeyState(VK_BACK) & 0x8000) != 0);
            io.AddKeyEvent(ImGuiKey_Enter, (GetAsyncKeyState(VK_RETURN) & 0x8000) != 0);

            // 4. Character input for search/text boxes
            static bool s_charKeysPrev[256] = { false };
            for (int vk = 'A'; vk <= 'Z'; vk++) {
                bool isDown = (GetAsyncKeyState(vk) & 0x8000) != 0;
                if (isDown && !s_charKeysPrev[vk]) {
                    io.AddInputCharacter((ImWchar)(shift ? vk : (vk + 32)));
                }
                s_charKeysPrev[vk] = isDown;
            }
            for (int vk = '0'; vk <= '9'; vk++) {
                bool isDown = (GetAsyncKeyState(vk) & 0x8000) != 0;
                if (isDown && !s_charKeysPrev[vk]) {
                    io.AddInputCharacter((ImWchar)vk);
                }
                s_charKeysPrev[vk] = isDown;
            }
            bool spaceDown = (GetAsyncKeyState(VK_SPACE) & 0x8000) != 0;
            if (spaceDown && !s_charKeysPrev[VK_SPACE]) {
                io.AddInputCharacter((ImWchar)' ');
            }
            s_charKeysPrev[VK_SPACE] = spaceDown;

            bool minusDown = (GetAsyncKeyState(VK_OEM_MINUS) & 0x8000) != 0;
            if (minusDown && !s_charKeysPrev[VK_OEM_MINUS]) {
                io.AddInputCharacter((ImWchar)(io.KeyShift ? '_' : '-'));
            }
            s_charKeysPrev[VK_OEM_MINUS] = minusDown;

            bool periodDown = (GetAsyncKeyState(VK_OEM_PERIOD) & 0x8000) != 0;
            if (periodDown && !s_charKeysPrev[VK_OEM_PERIOD]) {
                io.AddInputCharacter((ImWchar)'.');
            }
            s_charKeysPrev[VK_OEM_PERIOD] = periodDown;

            // Ensure RenderTargetView is valid
            if (!g_renderTargetView) {
                CreateRenderTarget(pSwapChain);
            }
            ...
            ImGui::Render();
            if (g_renderTargetView && g_d3d11Context) {
...
static DWORD WINAPI HookWorker(LPVOID) {
    ...
    if (target) {
        ...
        if (table) {
            ...
            // 8: Present
            if (table[8]) {
                if (VirtualProtect(&table[8], sizeof(void*), PAGE_READWRITE, &oldProtect)) {
                    g_originalPresent = reinterpret_cast<PresentFn>(table[8]);
                    table[8] = reinterpret_cast<void*>(HookPresent);
                    VirtualProtect(&table[8], sizeof(void*), oldProtect, &oldProtect);
                }
            }
            // 22: Present1
            if (swap1 && table[22]) {
                if (VirtualProtect(&table[22], sizeof(void*), PAGE_READWRITE, &oldProtect)) {
                    g_originalPresent1 = reinterpret_cast<Present1Fn>(table[22]);
                    table[22] = reinterpret_cast<void*>(HookPresent1);
                    VirtualProtect(&table[22], sizeof(void*), oldProtect, &oldProtect);
                }
            }
            // 13: ResizeBuffers
            if (table[13]) {
                if (VirtualProtect(&table[13], sizeof(void*), PAGE_READWRITE, &oldProtect)) {
                    g_originalResize = reinterpret_cast<ResizeBuffersFn>(table[13]);
                    table[13] = reinterpret_cast<void*>(HookResize);
                    VirtualProtect(&table[13], sizeof(void*), oldProtect, &oldProtect);
                }
            }
...
extern "C" __declspec(dllexport) void __cdecl SSJJUI_SetManagedTick(void* fn) {
    // Intentionally no-op: Managed commands are safely pumped on the Unity main thread in Update()
    (void)fn;
}
...
extern "C" __declspec(dllexport) void __cdecl SSJJUI_SetThirdPersonState(int enabled, float distance, float fov) {
    std::lock_guard<std::mutex> lock(g_stateMutex);
    g_thirdPersonEnabled = enabled != 0;
    g_thirdPersonDistance = distance;
    g_thirdPersonFov = fov;
}

extern "C" __declspec(dllexport) void __cdecl SSJJUI_SetWorldSnowState(int enabled, float density, float speed, float size) {
    std::lock_guard<std::mutex> lock(g_stateMutex);
    g_worldSnowEnabled = enabled != 0;
    g_worldSnowDensity = density;
    g_worldSnowSpeed = speed;
    g_worldSnowSize = size;
}
